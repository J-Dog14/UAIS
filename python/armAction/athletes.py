"""
Athlete management for Arm Action data.
...
def update_athletes_summary():
    ...
    This updates has_arm_action_data and arm_action_session_count flags.
    """
    # Update all athlete flags (including arm action flags)
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            if participant_name:
                ...
                SELECT 
                    ...
                    COUNT(DISTINCT aa.session_date) as total_sessions,
                    COUNT(aa.id) as total_movements,
                    MIN(aa.session_date) as first_session_date,
                    MAX(aa.session_date) as last_session_date
                FROM analytics.d_athletes a
                LEFT JOIN public.f_arm_action aa ON a.athlete_uuid = aa.athlete_uuid
                ...
                FROM analytics.d_athletes a
                LEFT JOIN public.f_arm_action aa ON a.athlete_uuid = aa.athlete_uuid
                WHERE a.has_arm_action_data = TRUE
                GROUP BY a.name, a.athlete_uuid
